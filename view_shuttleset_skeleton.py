...
frames = []
data = json.load(open('/mnt/ssd/zhaoyu/Skeleton/data/shuttleset/val.json'))
pause = True
...
for i, clip in enumerate(data):
    ...
    video_name = '/home/user/zhaoyu/IJCAI/badminton_videos/' + video_name + '.mp4'
    print(file_name)
    ...
    # clip_players = pd.read_csv('players_badminton/%s.csv' % file_name)

    if os.path.exists('skeletons/shuttleset/%s.pkl' % file_name):
        clip_skeletons = pd.read_pickle('skeletons/shuttleset/%s.pkl' % file_name)
    else:
        clip_skeletons = None
        print('MISSING!!! skeletons/shuttleset/%s.pkl' % file_name)
